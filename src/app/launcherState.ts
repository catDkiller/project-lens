...
export function deriveLauncherState(codex: CodexStatusDto | undefined, source: LauncherSource, model: ModelChoice, reading = false): CanonicalLauncherState {
  ...
  const available = codex?.models ?? []; const modelReady = Boolean(model && available.includes(model)); const disabledReason = view !== 'READY' ? view === 'EMPTY' ? 'Choose a project or use the prepared sample.' : view === 'READING_SOURCE' ? 'Reading the selected project.' : 'This project cannot be analysed yet.' : engine.status !== 'ready' ? engine.recovery ?? 'Codex is not ready.' : !modelReady ? available.length ? 'Choose a confirmed Codex model to continue.' : 'No account-compatible Codex model was discovered.' : undefined
  return { source, engine, model, view, canAnalyse: view === 'READY' && engine.status === 'ready' && modelReady, disabledReason, privacyDescription }
}
