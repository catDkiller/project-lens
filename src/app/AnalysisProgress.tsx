...
export function AnalysisProgress({ projectName = 'project', modelId, events, startedAt = Date.now(), failed, onCancel, onRetry, onChooseModel, onConnectOpenCode }: AnalysisProgressProps) {
  ...
  const stalled = !failed && sinceLastEvent >= 35_000
  ...
  return <main className="analysis-screen"><section className="analysis-panel" aria-live="polite"><p className="eyebrow">Project Lens</p><h1>{`Understanding “${projectName}”`}</h1><div className="analysis-stages">{stages.map(([label, types]) => { const active = latest && (types as readonly string[]).includes(latest.type); const done = latest?.type === 'completed' || (types as readonly string[]).includes('queued') && events.some((event) => event.type === 'preparing-evidence'); return <div className={`analysis-stage-row${active ? ' active' : ''}${done ? ' done' : ''}`} key={label}><span aria-hidden="true">{done ? '✓' : active ? '●' : '○'}</span><strong>{label}</strong></div> })}</div><p className="analysis-current" role="status">{latest?.message ?? 'Preparing the analysis…'}</p>{stalled && <p className="analysis-stalled" role="status">Still waiting for the selected provider… No new response has arrived for {formatDuration(sinceLastEvent)}.</p>}<p className="analysis-meta">Elapsed: {formatDuration(elapsed)} · Last activity: {formatDuration(sinceLastEvent)} ago</p><p className="analysis-model">Model: <code>{modelId ?? 'Selected model'}</code></p><div className="analysis-actions"><button className="quiet-button" type="button" onClick={onCancel}>Cancel</button></div>{details}</section></main>
}
