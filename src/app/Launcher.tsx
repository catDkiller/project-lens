...
import type { AgentStatusDto, ModelDto, ProviderAuthSessionDto, ProviderDto } from '../local-api/contracts'
...
export function Launcher({ agent, models = [], providers = [], authSession, runtimeStatus = 'Checking local runtime…', isAnalysing, analysisStage, error, appearance, accent, selectedProjectName, selectedProjectSummary, selectedProjectSkipped, webResearchEnabled, onAppearance, onAccent, onWebResearchEnabled, onTrySample, onUsePrepared, onImportLocal, onCancel, onRefreshProviders = () => {}, onRefreshModels = () => {}, onConnectProvider = () => {}, onDisconnectProvider = () => {}, onCheckConnection = () => {}, onCancelConnection = () => {} }: LauncherProps) {
  ...
  const [pendingCostModel, setPendingCostModel] = useState<ModelDto>()
  const isExplicitFree = (item: ModelDto) => item.cost === 'explicit-free' || item.free === true || item.explicitlyFree === true
  const readyFreeModels = readyModels.filter(isExplicitFree)
  const readyUsageModels = readyModels.filter((item) => !readyFreeModels.includes(item))
  useEffect(() => { if (confirmedCostModelId && confirmedCostModelId !== selected?.fullId) setConfirmedCostModelId(undefined); if (pendingCostModel && pendingCostModel.fullId !== selected?.fullId) setPendingCostModel(undefined) }, [confirmedCostModelId, pendingCostModel, selected?.fullId])
  ...
  useLayoutEffect(() => {
    ...
    const position = () => {
      ...
      if (!trigger) return
      const rect = trigger.getBoundingClientRect(); const margin = 12; const gap = 8; const width = Math.min(520, Math.max(320, window.innerWidth - margin * 2)); const estimatedHeight = Math.min(416, window.innerHeight - margin * 2); const below = window.innerHeight - rect.bottom - margin; const top = below >= estimatedHeight || below >= rect.top ? rect.bottom + gap : Math.max(margin, rect.top - estimatedHeight - gap); const left = Math.min(Math.max(margin, rect.right - width), window.innerWidth - width - margin)
      setPopupStyle({ top, left, width, maxHeight: `calc(100dvh - ${margin * 2}px)` })
    }
  ...
  const choose = (item?: ModelDto) => { if (!item) return; setModel(item.fullId); if (isReady(item)) { closePopup(); if (!isExplicitFree(item) && confirmedCostModelId !== item.fullId) setPendingCostModel(item); return } if (item.readiness === 'setup-required' || item.availability === 'requires-provider') { closePopup(); onConnectProvider(item.providerId) } }
  const startSelectedAnalysis = () => { if (!selected || !ready) return; if (!isExplicitFree(selected) && confirmedCostModelId !== selected.fullId) { setPendingCostModel(selected); return }; onTrySample(selected.fullId) }
  ...
  const popup = open ? createPortal(
    <div className="model-popover" style={popupStyle} role="dialog" aria-label="Choose model" onPointerDown={(event) => event.stopPropagation()}>
      ...
      <div className="model-results" role="listbox">
        <ModelGroup label="Ready · Free" models={readyFreeModels} selectedId={selected?.fullId} highlightedId={highlightedId} onHighlight={setHighlightedId} onChoose={choose} status="Ready · Free" />
        <ModelGroup label="Ready · May incur charges" models={readyUsageModels} selectedId={selected?.fullId} highlightedId={highlightedId} onHighlight={setHighlightedId} onChoose={choose} status="Ready · May incur charges" />
  ...
  return (
    <main className="launcher-screen">
      ...
      <section className="launcher-stage">
        ...
        {pendingCostModel && <div className="provider-connection" role="alert"><strong>This model may incur charges through your connected provider.</strong><div className="provider-actions"><button type="button" onClick={() => { setConfirmedCostModelId(pendingCostModel.fullId); setPendingCostModel(undefined); onTrySample(pendingCostModel.fullId) }}>Use this model</button><button type="button" onClick={() => { setPendingCostModel(undefined); setOpen(true) }}>Choose a free model</button></div></div>}
