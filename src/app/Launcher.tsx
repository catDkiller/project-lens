...
interface LauncherProps { agent?: AgentStatusDto; models: ModelDto[]; providers?: ProviderDto[]; runtimeStatus: string; isAnalysing: boolean; analysisStage?: AnalysisStageId; error?: string; appearance: Appearance; accent: Accent; selectedProjectName?: string; selectedProjectSummary?: string; onAppearance: (value: Appearance) => void; onAccent: (value: Accent) => void; onTrySample: (modelId: string) => void; onUsePrepared: () => void; onImportLocal: (name: string, files: ProjectFile[]) => void; onCancel: () => void; onRefreshProviders?: () => void; onRefreshModels?: () => void; onConnectProvider?: (providerId: string) => void; onDisconnectProvider?: (providerId: string) => void }
...
export function Launcher({ agent, models = [], providers = [], runtimeStatus = 'Checking local runtime…', isAnalysing, analysisStage, error, appearance, accent, selectedProjectName, selectedProjectSummary, onAppearance, onAccent, onTrySample, onUsePrepared, onImportLocal, onCancel, onRefreshProviders = () => {}, onRefreshModels = () => {}, onConnectProvider = () => {}, onDisconnectProvider = () => {} }: LauncherProps) {
  ...
  const selectedModelReady = selectedData?.runnable === true || selectedData?.availability === 'available'
  ...
  function chooseModel(id: string) { const option = models.find((item) => item.fullId === id); if (!option || (option.runnable === false || option.availability === 'requires-provider' || option.availability === 'unavailable')) return; setModel(id); setPickerOpen(false); setQuery('') }
  ...
  async function chooseFolder() {
    setFolderStatus(''); setReadingFolder(true)
    try {
    ...
    } catch (reason) { setFolderStatus(reason instanceof DOMException && reason.name === 'AbortError' ? 'Folder selection cancelled.' : 'The folder could not be prepared locally.') } finally { setReadingFolder(false) }
  }
