...
function event(run: Run, next: AnalysisEventDto, genuine = false) { const timestamp = new Date().toISOString(); run.lastAnyEventAt = timestamp; if (genuine) run.lastGenuineAgentEventAt = timestamp; run.events.push({ ...next, id: next.id ?? randomUUID(), runId: run.runId, sequence: run.events.length + 1, timestamp }) }
...
async function execute(run: Run) {
  ...
  setState(run, 'preparing-project'); event(run, { type: 'preparing-evidence', stage: 'preparing', status: 'active', message: 'Preparing local analysis.' })
  event(run, { type: 'preparing-evidence', stage: 'discovery', status: 'active', message: 'Scanning local project.' })
  const analysis = await runProjectAnalysis(project, [], (stage, state) => { if (state === 'running') event(run, { type: 'preparing-evidence', stage: stage === 'inventory' ? 'discovery' : 'filtering', status: 'active', message: stage === 'inventory' ? `Discovered ${project.files.length} readable project files.` : `Selecting relevant files and ${stage}.`, metadata: { discoveredFiles: project.files.length, includedFiles: project.files.length } }) }, 0)
  ...
  const staged = await stageRun(process.cwd(), run.runId, project.files, skill, (file) => event(run, { type: 'preparing-evidence', stage: 'snapshot', status: 'active', message: `Copied ${file.path}.`, progress: { current: file.index, total: file.total, unit: 'files', percentage: file.total ? Math.round(file.index / file.total * 100) : 100 }, file: { projectRelativePath: file.path, action: 'copied' }, metadata: { includedFiles: file.total, processedBytes: file.bytes } }))
