...
import { detectAgentRuntimes } from '../agents'
...
export const daemon = createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && url.pathname === '/api/agents') return send(res, 200, await Promise.all((await detectAgentRuntimes()).map(async (runtime) => ({ id: runtime.id, displayName: runtime.displayName, installed: runtime.installationStatus === 'installed', executablePath: runtime.executable, version: runtime.version, status: runtime.installationStatus === 'installed' ? 'available' : 'unavailable', readiness: runtime.readiness === 'ready' ? 'ready' : runtime.readiness === 'sign-in-required' ? 'needs-authentication' : runtime.readiness === 'not-installed' ? 'unavailable' : 'unhealthy', authenticationStatus: runtime.authenticationStatus, capabilities: runtime.capabilities, error: runtime.readiness === 'sign-in-required' ? 'Sign in required.' : undefined }))))
