...
export function createPresentationFallback(knowledge: ProjectKnowledgeBase): PresentationKnowledgeBase {
  ...
  return {
    ...
    files: files.map((file) => ({ path: file.path, explanation: file.purpose === 'Analysed project file.' ? 'The file was found, but its exact role could not be confirmed.' : file.purpose, itemType: file.itemType, analysisStatus: file.analysisStatus, preview: file.optionalPreview })),
