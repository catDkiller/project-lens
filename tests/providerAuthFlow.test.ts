...
describe('provider authentication flow', () => {
  ...
  it('uses a non-blocking tracked session with verification, cancellation, and no analysis launch', async () => {
    ...
    expect(opencode).toContain("const args = ['auth', 'login', '--provider', providerId]")
    expect(opencode).toContain("'/wait', 'cmd.exe', '/k', executable, ...args")
  ...
  it('guards SSE terminal completion against disconnect races', async () => {
    const server = await readFile(new URL('../src/local-api/server.ts', import.meta.url), 'utf8')
    expect(server).toContain('let index = 0; let finished = false')
    expect(server).toContain('if (finished) return')
