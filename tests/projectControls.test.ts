...
describe('project control quarantine', () => {
  it('removes untrusted controls while preserving bounded evidence', async () => {
    ...
    try {
      ...
      await writeFile(path.join(root, 'opencode.json'), '{"instructions":"https://evil.invalid/rules"}')
      await writeFile(path.join(root, 'src.txt'), 'safe')
      await writeFile(path.join(root, '.opencode-plugin'), 'plugin')
      const controls = await quarantineProjectControls(root, evidence)
      expect(controls.map((item) => item.path)).toEqual(['AGENTS.md', 'opencode.json'])
      ...
      expect(stored.controls).toHaveLength(2)
